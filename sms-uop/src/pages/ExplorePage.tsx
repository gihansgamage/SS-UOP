...
const ExplorePage: React.FC = () => {
  ...
  const [filteredSocieties, setFilteredSocieties] = useState<Society[]>([]); // Initialize empty
  ...
  useEffect(() => {
    // 1. Guard against undefined societies list
    if (!societies) return;

    let filtered = [...societies];

    // 2. Search Filter (With Safety Checks)
    if (searchTerm) {
      const lowerTerm = searchTerm.toLowerCase();
      filtered = filtered.filter(society =>
          (society.societyName && society.societyName.toLowerCase().includes(lowerTerm)) ||
          (society.president?.name && society.president.name.toLowerCase().includes(lowerTerm)) ||
          (society.secretary?.name && society.secretary.name.toLowerCase().includes(lowerTerm))
      );
    }
    ...
    if (statusFilter !== 'all') {
      filtered = filtered.filter(society =>
          society.status && society.status.toLowerCase() === statusFilter.toLowerCase()
      );
    }
    ...
    if (yearFilter !== 'all') {
      filtered = filtered.filter(society =>
          society.year && society.year.toString() === yearFilter
      );
    }
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'date':
          return new Date(b.registeredDate || 0).getTime() - new Date(a.registeredDate || 0).getTime();
        case 'year':
  ...
  // Safely extract available years
  const availableYears = societies
      ? [...new Set(societies.map(s => s.year).filter(y => y))].sort((a, b) => b - a)
      : [];
                            ...
                            <div className="p-6">
                              <div className="flex justify-between items-start mb-4">
                                ...
                                <span className={`px-2 py-1 text-xs font-medium rounded-full flex-shrink-0 ml-2 ${
                                    (society.status || '').toLowerCase() === 'active' ? 'bg-green-100 text-green-800' :
                                        (society.status || '').toLowerCase() === 'pending' ? 'bg-yellow-100 text-yellow-800' :
                                            'bg-gray-100 text-gray-800'
                                }`}>
                          {(society.status || 'UNKNOWN').toUpperCase()}
                        </span>
                              </div>
                              ...
                              <div className="space-y-3 text-sm">
                                ...
                                <div className="flex justify-between">
                                  ...
                                  <span className="font-medium">{society.registeredDate ? new Date(society.registeredDate).toLocaleDateString() : 'N/A'}</span>
                                </div>
                                ...
                                <div className="border-t pt-3">
                                  ...
                                  <div className="space-y-1">
                                    <div className="text-xs">
                                      ...
                                      {/* SAFE NAVIGATION for President */}
                                      <span className="font-medium ml-1">{society.president?.name || 'N/A'}</span>
                                    </div>
                                    <div className="text-xs">
                                      <span className="text-gray-600">Secretary:</span>
                                      {/* SAFE NAVIGATION for Secretary */}
                                      <span className="font-medium ml-1">{society.secretary?.name || 'N/A'}</span>
                                    </div>
                                  </div>
                                </div>
                              ...
                              <tr key={society.id} className="hover:bg-gray-50 transition-colors">
                                <td className="px-6 py-4">
                                  <div>
                                    ...
                                    <div className="text-sm text-gray-500">{society.registeredDate ? new Date(society.registeredDate).toLocaleDateString() : 'N/A'}</div>
                                  </div>
                                </td>
                                <td className="px-6 py-4">
                                  <div className="text-sm text-gray-900">{society.president?.name || 'N/A'}</div>
                                  <div className="text-sm text-gray-500">{society.president?.email || ''}</div>
                                </td>
                                <td className="px-6 py-4">
                                  <div className="text-sm text-gray-900">{society.secretary?.name || 'N/A'}</div>
